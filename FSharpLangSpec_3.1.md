...
#### 4.4.2 記号演算子とパターン/式構築の優先順位

優先順位の規則は、曖昧な表現とパターン構築のための評価順序を制御します。
優先順位の高い項目は優先順位の低い項目の前に評価されます。
以下の表は、最高から最低までの優先順位を示し、演算子や式はその左右のトークンに関連付けられているかどうかを示します。
`OP` マーカーは、表にリストされているものを除く、指定した接頭辞で始まる `symbolic-op` トークンを表します。
例えば `+OP` は、トークンが他の場所で表に現われない限り、プラス記号で始まるトークンを表します。

| 演算子または式 | 結合性 | コメント |
| -------------- | ------ | -------- |
| f<types> | Left | High-precedence type application; see [セクション15.3](#section15-3) |
| f(x) | Left | High-precedence application; see [セクション15.2](#section15-2) |
| . | Left |  |
...
([セクション6](#section6)からの規則など)曖昧な文法規則は表内のトークンを伴い、以前の表よりも早く現われる構文は、後から表に現れる構文よりも優先順位が高いです。
結合性は、演算子または構文が、演算子の左側か右側の項目に適用されるかどうかを示します。

例えば、次のトークンストリームを考えてください:

```fsharp
a + b * c
```

この式では、`*` 演算子は `+` 演算子よりも優先順位が高いので、`b * c` のための `expr infix-op expr` 規則は、 `a + b` のための `expr infix-op expr` 規則よりも優先されます。
このように、この式は次のように描写できます:
...
`*` の左結合性は、我々が曖昧さなどの決断を描写できることを意味します。
...
上記の表では、中置演算子の優先順位を決定するとき、主要な `.` 文字は無視されます。
例えば、 `.*` は `*` と同じ優先順位を持っています。
この規則は、次のような `.*` 演算子は、マトリクス上の pointwise-operation に頻繁に利用され、期待する優先順位を持つことを保証します。

"高優先順位のアプリケーション"と"高優先順位型アプリケーション"としてマークされたこの表項目は、 [セクション15.2](#section15-2)と[セクション15.3](#section15-3)で説明したように、字句トークンストリームの増強の結果です。
